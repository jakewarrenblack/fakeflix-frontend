...
const TitleCard = ({title, showRemover=false, setReload, type}) => {
    ...
    const BackgroundContainer = ({children}) => {
        ...
        if(!image && imageLoading){
            // return loader
            Container = <div className={'h-[225px] group relative rounded z-0 flex flex-col justify-end bg-cover bg-no-repeat bg-center'} style={{background: 'rgba(7,7,8, 1)'}}>
                <Oval
                    height={40}
                    width={40}
                    color="#CC0000"
        ...
        else if(!image && !imageLoading){
            // return placeholder, failed to fetch an image
            Container = <div className={'h-[225px] group relative rounded z-0 flex flex-col justify-end bg-cover bg-no-repeat bg-center'} style={{background: 'rgba(7,7,8, 1)'}}>{children}</div>
        }
        else{
            // return image
            Container = <div className={'h-[225px] group relative rounded z-0 flex flex-col justify-end bg-cover bg-no-repeat bg-center'} style={{background: `no-repeat center/cover url(${image}`}}>{children}</div>
        }
